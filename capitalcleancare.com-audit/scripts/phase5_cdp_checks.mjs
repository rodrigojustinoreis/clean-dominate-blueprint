// Phase 5: hydration + layout + screenshots via headless Chrome (CDP 9350).
// Usage: node phase5_cdp_checks.mjs <base-url> <out-dir>
import { writeFileSync, mkdirSync } from "node:fs";
const [BASE, OUT] = process.argv.slice(2); mkdirSync(OUT, { recursive: true });
const PATH = "/resources/how-to-clean-hardwood-floors-after-construction";
...
for(const [vp,w,h,mob] of [["desktop",1440,900,false],["mobile",390,844,true]]){errs=[];
  ...
  writeFileSync(`${OUT}/phase5-${vp}-fold.jpg`,Buffer.from(shot.data,"base64"));
  ...
  writeFileSync(`${OUT}/phase5-${vp}-full.jpg`,Buffer.from(full.data,"base64"));
  out[vp]={errors:errs.slice(),...r};
  console.log(`${vp}: react/console errors=${errs.length?errs.join(" | "):"none"} h1=${r.h1} ovf=${r.ovf}px broken=${JSON.stringify(r.broken)} hero=${JSON.stringify(r.hero)} video=${JSON.stringify(r.video)} schemas=${r.schemas.join(",")} robots="${r.robots}"`);
}
writeFileSync(`${OUT}/phase5-cdp-results.json`,JSON.stringify(out,null,2));
