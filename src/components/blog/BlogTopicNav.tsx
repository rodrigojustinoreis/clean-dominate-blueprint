...
// Pill navigation for the blog. Rendered on /blog (active=undefined → "All Posts")
// and on each /blog/topic/<slug> hub (active=slug). Each pill is a real <Link> to a
  ...
  return (
    <nav className="-mx-4 px-4 mb-10 overflow-x-auto scrollbar-none" aria-label="Browse blog by topic">
      <div className="flex items-center gap-2 w-max">
        <Link to="/blog" className={`${base} ${!active ? on : off}`} aria-current={!active ? "page" : undefined}>
          All Posts
        </Link>
        {BLOG_TOPICS.map((t) => (
          <Link
