...
export function getServiceLocationFAQs(city: ServiceLocationCity, service: ServiceLocationService): { q: string; a: string }[] {
  ...
  const serviceSpecificFaqs: Record<string, { q: string; a: string }[]> = {
    ...
    "best-cleaning-company": [
      ...
      { q: `Are your ${city.name} reviews actually verified?`, a: `Yes. Our Google reviews are all from verified ${city.name} clients. Each review is tied to a real cleaning visit. Read them — they describe specific details that can only come from actual service experiences.` },
