...
describe("submitLeadDual — e-mail critical, db backup", () => {
  it("email 200 success:true + db ok → accepted/accepted, emailJustAccepted once", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1); expect(insertImpl).toHaveBeenCalledTimes(1);
  });
  it("email 200 success:\"true\" (string) → accepted", async () => {
    stubFetch(() => Promise.resolve(okBody({ success: "true" }))); insertImpl.mockResolvedValue({ error: null });
    expect((await run()).email).toBe("accepted");
  });
  it("email 200 success:false / invalid JSON / null body → failed (unconfirmed)", async () => {
    insertImpl.mockResolvedValue({ error: null });
    stubFetch(() => Promise.resolve(okBody({ success: false }))); expect((await run()).email).toBe("failed");
    ref.current = null; stubFetch(() => Promise.resolve({ ok: true, json: () => Promise.reject(new SyntaxError("bad")) })); expect((await run()).email).toBe("failed");
    ref.current = null; stubFetch(() => Promise.resolve(okBody(null))); expect((await run()).email).toBe("failed");
  });
  it("email accepted + db failed → still accepted (backup only)", async () => {
    stubFetch(() => Promise.resolve(okBody({ success: true }))); insertImpl.mockResolvedValue({ error: { message: "db down" } });
  ...
  it("email accepted even when the Supabase import rejects or the insert throws", async () => {
    stubFetch(() => Promise.resolve(okBody({ success: true })));
  ...
  it("email 500 + db accepted → unconfirmed; manual retry sends only the e-mail (no second insert)", async () => {
    let n = 0; stubFetch(() => Promise.resolve(n++ === 0 ? res500() : okBody({ success: true }))); insertImpl.mockResolvedValue({ error: null });
    ...
    expect(fetchMock).toHaveBeenCalledTimes(2); expect(insertImpl).toHaveBeenCalledTimes(1);
  });
  it("email rejected + db failed → both failed; retry tries both again", async () => {
    ...
    const r2 = await run(); expect(fetchMock).toHaveBeenCalledTimes(2); expect(insertImpl).toHaveBeenCalledTimes(2); expect(r2.dbSkipped).toBe(false);
  });
